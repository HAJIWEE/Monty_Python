class ToDo:
    ...
    comTd = 0  # Tracking progress of todo tasks
...
class Deadline(ToDo):
    ...
    comDL = 0 # Tracking progress of deadline tasks
    ...
    def value(self):
        d = 'Done' if self.done else '-'
        # formatting Deadline.value to return a string value in the format to display on the GUI
        return f'{d}'.center(7, ' ') + '|' + f'{self.idx}'.center(7, ' ') + '| ' + ' '.join(self.description).ljust(28,' ') + '| ' + ''.join(self.deadline).center(13, ' ')

    def com(self):
        d = 'Done' if self.done else '-'
        # formatting Deadline.com to return a list format to save in comma seperated file format
